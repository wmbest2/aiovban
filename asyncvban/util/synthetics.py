...
def named_synthetic(name, mask=0xff, offset=0, **kwargs) -> Field:
    return field(metadata={f"{SYNTHETIC_NAME}": name, "mask": mask, "offset": offset}, **kwargs)
...
class SyntheticMixin:
    ...
    def __post_init__(self):
        ...
        masks_by_name = { k: list(v) for k, v in groupby(self.__dataclass_fields__.values(), key=lambda x: x.metadata.get(SYNTHETIC_NAME)) }
        ...
        for (synthetic, fields) in masks_by_name.items():
            ...
            def getter(s, fields=fields):
                return functools.reduce(lambda x, y: x | (int(getattr(s, y.name)) & y.metadata[MASK]) - y.metadata["offset"], fields, 0)
            ...
            setattr(self.__class__, synthetic, property(fget=getter, fset=setter, fdel=deleter))
