...
class VBANAudioPlayer:
    ...
    async def handle_packets(self, packets: [VBANPacket]):
        if packets:
            ...
            await asyncio.get_running_loop().run_in_executor(None, self._stream.write, data)
    async def _stream_audio(self, stream_waiter: asyncio.Future):
        async def gather_frames(frame_count):
            return [await self.stream.get_packet() for _ in range(frame_count)]
        ...
        try:
            ...
            stream_waiter.set_exception(e)

    async def listen(self):
        self._stream.start_stream()
        stream_waiter = asyncio.Future()
        asyncio.get_running_loop().create_task(self._stream_audio(stream_waiter))
        await stream_waiter
