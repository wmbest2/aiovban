...
class Ping(PacketBody, VBANApplicationData):
    ...
    def unpack(cls, data):
        ...
        if len(data) < expected_size:
            raise ValueError(f"Insufficient data for Ping packet: expected {expected_size} bytes, got {len(data)}")
        
        try:
            unpacked_data = struct.unpack(PING_STRUCT_FORMAT, data[:expected_size])
        except struct.error as e:
            raise ValueError(f"Failed to unpack Ping packet: {e}")
        ...
        try:
            return cls(
                ...
                gps_position=unpacked_data[11].decode("ascii", errors="ignore").strip("\x00"),
                user_position=unpacked_data[12].decode("ascii", errors="ignore").strip("\x00"),
                lang_code=unpacked_data[13].decode("ascii", errors="ignore").strip("\x00"),
                reserved=unpacked_data[14],
                reserved_ex=unpacked_data[15],
                distant_ip=unpacked_data[16].decode("ascii", errors="ignore").strip("\x00"),
                distant_port=unpacked_data[17],
                distant_reserved=unpacked_data[18],
                device_name=unpacked_data[19].decode("ascii", errors="ignore").strip("\x00"),
                manufacturer_name=unpacked_data[20].decode("ascii", errors="ignore").strip("\x00"),
                application_name=unpacked_data[21].decode("ascii", errors="ignore").strip("\x00"),
                host_name=unpacked_data[22].decode("ascii", errors="ignore").strip("\x00"),
                user_name=unpacked_data[23].decode("utf-8", errors="ignore").strip("\x00"),
                user_comment=unpacked_data[24].decode("utf-8", errors="ignore").strip("\x00"),
            )
        except (UnicodeDecodeError, ValueError) as e:
            raise ValueError(f"Failed to decode Ping packet fields: {e}")
