...
class TestRTPacketBodyType0(unittest.TestCase):
    ...
    def setUp(self):
        self.sample_data = {
            ...
            "strips": [
                Strip(
                    label=f"Strip {i}" + "-" * random.randint(0, 100),
                    state=generate_random_state(),
                    layers=generate_random_levels(8),
                )
                for i in range(8)
            ],
            "buses": [
                Bus(
                    label=f"Bus {i}" + "-" * random.randint(0, 100),
                    state=generate_random_state(),
                    gain=random.randint(0, 65535),
                )
                for i in range(8)
            ],
        }
    ...
    def test_pack_unpack(self):
        ...
        for i in range(8):
            ...
            self.assertEqual(
                ...
                self.sample_data["strips"][i].label[:60],
            )
            self.assertEqual(
            ...
            self.assertEqual(
                ...
                self.sample_data["buses"][i].label[:60],
            )
            self.assertEqual(
